...
def get_employee_info_out(rfid):
    ...
    statement = "SELECT RFID.ID_RFID, Contractor.IDCard, Contractor.contractno, Contractor.FirstName, Contractor.LastName FROM dbo.RFID AS RFID INNER JOIN dbo.Contractor AS Contractor ON RFID.IDCard = Contractor.IDCard  WHERE (RFID.ID_RFID LIKE '%{0}%')".format(rfid)
    conn = pyodbc.connect('DRIVER={0};SERVER={1};DATABASE={2};UID={3};PWD={4}'.format(driver, server_name, database, uid, password))
    cursor = conn.cursor()
    cursor.execute(statement)
    return cursor.fetchone()


def get_employee_info_in(rfid):
    connection_info = config('config.ini', 'Database')
    driver = connection_info['driver']
    server_name = connection_info['server_name']
    database = connection_info['database_name']
...
if __name__ == '__main__':
    ...
    # print(get_employee_info_in('142F'))
    print(get_employee_info_out('2c862'))
